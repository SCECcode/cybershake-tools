...
RUN_DIR = "/home/shock/scottcal/runs"
ROOT_DIR = "%s/Study_21.12_management" % (RUN_DIR)
SCRIPT_DIR = "%s/cybershake-tools/scripts" % (RUN_DIR)

SGT = 0
PP = 1
FULL = 2

PENDING_FILE = "%s/pending.txt" % ROOT_DIR
...
def initExecutionSites():
    ...
    # Summit
    summit_site = ExecutionSite("summit", 20, full=True, SGT=True, PP=True)
    summit_site.setX509Proxy("/home/shock-ssd/scottcal/condor/x509up_u7588")
    sites.append(summit_site)
    # Blue Waters
    return sites
...
def getRunsToSubmit(pendingList, cursor):
    ...
    if output[0]!=b"" or output[1]!=b"": #something must have failed
#   if output!="":
    ...
    if sum(capacity)==0:
        ...
        while sum(capacity)>0 and index<len(pendingList) and len(sitesToSubmit)<MAX_SIMUL_SUBMIT:
            ...
            if len(pieces)==0:
                ...
                continue
            if pieces[0][0]=="#":
                # It's a comment line, ignore, but leave in so it gets written back to the file
                index += 1
                continue
            flag = 0
            ...
            for runningID in runningRunIDs:
                ...
                run_query = "select S.CS_Short_Name, R.ERF_ID, R.Rup_Var_Scenario_ID, R.SGT_Variation_ID, R.Velocity_Model_ID, R.Low_Frequency_Cutoff, R.Max_Frequency, R.SGT_Source_Filter_Frequency from CyberShake_Runs R, CyberShake_Sites S where R.Site_ID=S.CS_Site_ID"
                ...
                if db_run_data[0]==pieces[1] and db_run_data[1]==pieces[2] and db_run_data[2]==pieces[3] and db_run_data[3]==pieces[4] and db_run_data[4]==pieces[5] and db_run_data[5]==pieces[6] and db_run_data[6]==pieces[7] and db_run_data[7]==pieces[8]:
                    flag = 1
                    pendingList.pop(index)
                    break
            if flag==1:
    ...
    for run in runsToSubmit:
        ...
        if not restart:
            ...
            if workflow_type=="SGT":
                createString = "%s/create_sgt_dax.sh -s %s -v %s -e %d -r %d -g %d -f %f -q %f" % (SCRIPT_DIR, site, vel_model, erf_id, rv_id, sgt_id, freq, src_freq)
                sgtArgs = "%s %s" % (global_sgt_args, " ".join(execution_site[0].SGT_args))
                if len(sgtArgs)>0:
                    createString = "%s --sgtargs %s" % (createString, sgtArgs)
                executeString = "%s; %s" % (sourceString, createString)
                print(executeString, file=sys.stderr)
                rc = os.system(executeString)
                if rc!=0:
                    print("Error executing %s, aborting." % executeString, file=sys.stderr)
                    return 1
                run_table_file = "%s/%s_SGT_dax/run_table.txt" % (RUN_DIR, site)
            elif workflow_type=="PP":
                createString = "%s/create_pp_wf.sh -s %s -v %s -e %d -r %d -g %d -f %f -q %f" % (SCRIPT_DIR, site, vel_model, erf_id, rv_id, sgt_id, freq, src_freq)
                ...
                if rc!=0:
                    print("Error executing %s, aborting." % executeString, file=sys.stderr)
                    return 1
                run_table_file = "%s/%s_PP_dax/run_table.txt" % (RUN_DIR, site)
            elif workflow_type=="full":
                createString = "%s/create_full_wf.sh -s %s -v %s -e %d -r %d -g %d -f %f -q %f" % (SCRIPT_DIR, site, vel_model, erf_id, rv_id, sgt_id, freq, src_freq)
                sgtArgs = "--sgtargs -ss %s" % (execution_site[0].name)
                if len(execution_site[0].SGT_args):
                    sgtArgs = "%s %s %s" % (sgtArgs, global_sgt_args, " ".join(execution_site[0].SGT_args))
                ...
                if rc!=0:
                    print("Error executing %s, aborting." % executeString, file=sys.stderr)
                    return 1
                run_table_file = "%s/%s_Integrated_dax/run_table.txt" % (RUN_DIR, site)
            else:
        ...
        if not restart:
            if workflow_type=="SGT":
                planString = "%s/plan_sgt.sh %s %d %s" % (SCRIPT_DIR, site, run_id, execution_site_name)
            elif workflow_type=="PP":
                planString = "%s/plan_pp.sh %s %d %s" % (SCRIPT_DIR, site, run_id, execution_site_name)
            elif workflow_type=="full":
                # We want to use the site name from the SGT part
                planString = "%s/plan_full.sh %s %d %s" % (SCRIPT_DIR, site, run_id, execution_sites[0].name)
            else:
                logFP.write("Don't recognize workflow type %s, skipping." % workflow_type)
                continue
        ...
        if restart:
            restartString = "-r"
        if workflow_type=="SGT":
            runString = "%s/run_sgt.sh %s %s %d" % (SCRIPT_DIR, restartString, site, run_id)
        elif workflow_type=="PP":
            runString = "%s/run_pp.sh %s %s %d" % (SCRIPT_DIR, restartString, site, run_id)      
        elif workflow_type=="full":
            runString = "%s/run_full.sh %s %s %d" % (SCRIPT_DIR, restartString, site, run_id)
        else:
            logFP.write("Don't recognize workflow type %s, skipping." % workflow_type)
            continue
