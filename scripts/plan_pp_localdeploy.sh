...
propfile=/home/scec-02/cybershk/runs/config/properties.localdeploy.pp
...
echo "pegasus-plan -Dpegasus.local.env=X509_USER_PROXY=${NEW_PROXY} -Dpegasus.properties=${newpropfile} --rescue 100 --dax ${DAX} -s $REMOTE_SITE,opensha -D dags -f --cluster horizontal -v --nocleanup"
pegasus-plan -Dpegasus.properties=${newpropfile} --rescue 100 --dax ${DAX} -s $REMOTE_SITE,opensha -D dags -f --cluster horizontal -vvvvv --nocleanup | tee log-plan-${DAX}-${REMOTE_SITE}


# Modify the run-time user properties file
line=`grep pegasus-run log-plan-${DAX}-${REMOTE_SITE} | cut -d " " -f2`
propfile=${line#*=}
...
grep $REMOTE_SITE $TC_PATH >> ${EXEC_DIR}/metadata.local
