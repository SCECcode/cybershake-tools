...
parser.add_argument("-d", "--db_info", help="Database configuration file.")
parser.add_argument("-s", "--server", help="Database host (default is moment-carc)")
...
db_info = "/home/shock/scottcal/runs/config/db_pass.txt"
...
with open(db_info, "r") as fp_in:
    pieces = fp_in.readline().split(":")
    username = pieces[0]
    password = pieces[1].strip()
    fp_in.close()

conn = pymysql.connect(host=server, db='CyberShake', passwd=password, user=username)
cur = conn.cursor()
