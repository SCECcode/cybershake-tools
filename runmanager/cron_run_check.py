...
def createCompCurves(rm, m):
    ...
    # Query for list of runs in plotting state; turn off lock
    run_list = rm.getRunsByState([PLOT_STATE], lock=False)
    ...
    for run in run_list:
        ...
        if (stats == None):
            ...
            period_list = ''
            for c in stats.getCurveList():
                #For now, just consider Rot50 curves
...
def main():
    ...
    sys.stderr.write("-- Starting up at %s --\n" % (time.strftime("%Y-%m-%d %H:%M:%S")))

    #Try locking 'lock' in the executable directory; if you can't then a previous instance is running and abort.
    exe_dir = os.path.dirname(sys.argv[0])
    fp = open("%s/lock" % exe_dir, "w")
    try:
        fcntl.lockf(fp, fcntl.LOCK_EX | fcntl.LOCK_NB)
    except IOError:
        sys.stderr.write("Cron run check is aborting, since another instance is already running.\n")
        sys.exit(-1)
