...
if [ $TWO_LEVELS -eq 1 ]; then
	...
	echo "pegasus-plan -vv --conf=${propfile} --dax $TOP_DAX --rescue 100 -s $REMOTE_SITE,shock -o ${REMOTE_SITE} --dir dags -f --nocleanup --output-dir data/SgtFiles/${SITE} | tee log-plan-${TOP_DAX}-${REMOTE_SITE}"
        pegasus-plan -vv --conf=properties.sgt.onesite --dax $TOP_DAX --rescue 100 -s $REMOTE_SITE,shock -o $REMOTE_SITE --dir dags -f --nocleanup --output-dir data/SgtFiles/${SITE} | tee log-plan-${TOP_DAX}-${REMOTE_SITE}
	if [ $? != 0 ]; then
        	echo "Failed to plan workflow."
        	exit 1
	fi
